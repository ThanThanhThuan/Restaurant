...
Public Class frmStockAdjustment_Warehouse
    Dim str As String
    Private Sub auto()
        Try
            ...
            con.Open()
            Dim sql As String = ("SELECT MAX(SA_ID) FROM StockAdjustment_Warehouse")
    ...
    Private Sub DeleteRecord()
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cq As String = "delete from StockAdjustment_Warehouse where SA_ID=@d1"
            ...
            If RowsAffected > 0 Then
                If rbPlus.Checked = True Then
                    con = New SqlConnection(cs)
                    con.Open()
                    Dim cb1 As String = "Update Temp_Stock set qty=qty - " & Val(txtQty.Text) & " where ProductID=@d1 and Warehouse=@d3"
                    cmd = New SqlCommand(cb1)
                    cmd.Parameters.AddWithValue("@d1", Val(txtProductID.Text))
                    cmd.Parameters.AddWithValue("@d3", cmbWarehouse.Text)
                    cmd.Connection = con
                    cmd.ExecuteNonQuery()
                    con.Close()
                End If
                If rbMinus.Checked = True Then
                    ...
                    con.Close()
                End If
                LogFunc(lblUser.Text, "Deleted the Stock Adjustment(W) record having adjustment id '" & txtAdjustmentID.Text & "'")
    ...
    Private Sub btnSave_Click(sender As System.Object, e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into StockAdjustment_Warehouse(SA_ID, ProductID, Date, AdjustmentType, Qty, Reason,Warehouse) VALUES (@d1,@d2,@d4,@d5,@d6,@d7,@d8)"
            ...
            cmd.Parameters.AddWithValue("@d8", cmbWarehouse.Text)
            cmd.Connection = con
            cmd.ExecuteNonQuery()
            con.Close()
            If rbPlus.Checked = True Then
            ...
            If rbMinus.Checked = True Then
                ...
                con.Close()
            End If
            LogFunc(lblUser.Text, "added the new Stock Adjustment(W) record having adjustment id '" & txtAdjustmentID.Text & "'")
    ...
    Sub fillCombo()
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim ctn1 As String = "SELECT ProductName FROM Product,Temp_Stock where Product.PID=Temp_Stock.ProductID and Warehouse=@d1 order by 1"
            cmd = New SqlCommand(ctn1)
            cmd.Parameters.AddWithValue("@d1", cmbWarehouse.Text)
            cmd.Connection = con
    ...
    Private Sub cmbProductName_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cmbProductName.SelectedIndexChanged
        Try
            ...
            cmd.CommandText = "SELECT PID,RTRIM(ProductCode) from Product,Temp_Stock where Product.PID=Temp_Stock.ProductID and ProductName=@d1 and Warehouse=@d2"
            cmd.Parameters.AddWithValue("@d1", cmbProductName.Text)
            cmd.Parameters.AddWithValue("@d2", cmbWarehouse.Text)
            rdr = cmd.ExecuteReader()
            If rdr.Read() Then
    ...
    Sub GetQty_S()
        Try
            ...
            con.Open()
            cmd = con.CreateCommand()
            cmd.CommandText = "Select IsNull(Sum(Qty),0),RTRIM(Unit) from Temp_Stock,Product where Temp_Stock.ProductID=Product.PID and ProductID=@d1 and Warehouse=@d2 group by ProductID,Unit"
            cmd.Parameters.AddWithValue("@d1", Val(txtProductID.Text))
            cmd.Parameters.AddWithValue("@d2", cmbWarehouse.Text)
            rdr = cmd.ExecuteReader()
            If rdr.Read() Then
    ...
    Private Sub btnUpdate_Click(sender As System.Object, e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "Update StockAdjustment_Warehouse set ProductID=@d2, Date=@d4, AdjustmentType=@d5, Qty=@d6, Reason=@d7,Warehouse=@d8 where SA_ID=@d1"
            cmd = New SqlCommand(cb)
            cmd.Parameters.AddWithValue("@d1", Val(txtAdjustmentID.Text))
            cmd.Parameters.AddWithValue("@d2", Val(txtProductID.Text))
            cmd.Parameters.AddWithValue("@d4", dtpDate.Value.Date)
            ...
            If rbPlus.Checked = True Then
                con = New SqlConnection(cs)
                con.Open()
                Dim cb1 As String = "Update Temp_Stock set qty=qty + ( " & Val(txtQty.Text) & " - " & Val(txtQ.Text) & ") where ProductID=@d1 and Warehouse=@d2"
                cmd = New SqlCommand(cb1)
                cmd.Parameters.AddWithValue("@d1", Val(txtProductID.Text))
                cmd.Parameters.AddWithValue("@d2", cmbWarehouse.Text)
                cmd.Connection = con
            ...
            If rbMinus.Checked = True Then
                ...
                con.Close()
            End If
            LogFunc(lblUser.Text, "Updated the Stock Adjustment(W) record having adjustment id '" & txtAdjustmentID.Text & "'")
    ...
    Private Sub btnGetData_Click(sender As System.Object, e As System.EventArgs) Handles btnGetData.Click
        frmStockAdjustment_Warehouse_Record.lblSet.Text = "SA"
        frmStockAdjustment_Warehouse_Record.Reset()
        frmStockAdjustment_Warehouse_Record.ShowDialog()
    End Sub

    Private Sub frmStockAdjustment_Warehouse_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        FillWarehouse()
    End Sub

    Private Sub cmbWarehouse_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cmbWarehouse.SelectedIndexChanged
