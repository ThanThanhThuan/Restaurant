...
Public Class frmRestaurantPOSEBRecord
    Sub fillBillNo()
        Try
            Dim CN As New SqlConnection(cs)
            CN.Open()
            adp = New SqlDataAdapter()
            adp.SelectCommand = New SqlCommand("SELECT distinct RTRIM(BillNo) FROM RestaurantPOS_BillingInfoEB order by 1", CN)
            ds = New DataSet("ds")
            adp.Fill(ds)
            dtable = ds.Tables(0)
    ...
    Public Sub GetData()
        Try
            ...
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoEB.Id), RTRIM(BillNo),BillDate,EBDiscountPer,EBDiscountAmt,(RestaurantPOS_BillingInfoEB.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate,RTRIM(EB_Status),RTRIM(EB_PhoneNo),RTRIM(Member_ID) from RestaurantPOS_BillingInfoEB where (DATEDIFF(d,BillDate,GetDate())= 0) Order by BillDate Desc"
    ...
    Private Sub dgw_MouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dgw.MouseClick
        Try
            ...
            If dgw.Rows.Count > 0 Then
                ...
                'End If
                'If (lblUserType.Text = "Super Admin" And dr.Cells(12).Value.ToString() <> "Void") Or (lblUserType.Text = "Admin" And dr.Cells(12).Value.ToString() <> "Void") Then
                ...
                frmPOS.Calc3()
                If dr.Cells(12).Value = "Unpaid" Then
                    'frmPOS.flpEB.Visible = True
                    frmPOS.fillCurrencyEB()
                Else
                    'frmPOS.flpEB.Visible = False
                End If
                'frmPOS.txtEBContactNo.Text = dr.Cells(13).Value.ToString()
                frmPOS.txtStatus.Text = dr.Cells(12).Value.ToString()
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoEB.Id), RTRIM(BillNo),BillDate,EBDiscountPer,EBDiscountAmt,(RestaurantPOS_BillingInfoEB.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate,RTRIM(EB_Status),RTRIM(EB_PhoneNo),RTRIM(Member_ID) from RestaurantPOS_BillingInfoEB where  BillDate >=@d1 and BillDate < @d2 Order by BillDate"
            cmd = New SqlCommand(sql, con)
            cmd.CommandTimeout = 0
    ...
    Private Sub cmbBillNo_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbBillNo.SelectedIndexChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoEB.Id), RTRIM(BillNo),BillDate,EBDiscountPer,EBDiscountAmt,(RestaurantPOS_BillingInfoEB.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate,RTRIM(EB_Status),RTRIM(EB_PhoneNo),RTRIM(Member_ID) from RestaurantPOS_BillingInfoEB where BillNo=@d1 Order by BillDate"
            cmd = New SqlCommand(sql, con)
            cmd.CommandTimeout = 0
    ...
    Private Sub txtContactNo_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtContactNo.TextChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoEB.Id), RTRIM(BillNo),BillDate,EBDiscountPer,EBDiscountAmt,(RestaurantPOS_BillingInfoEB.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate,RTRIM(EB_Status),RTRIM(EB_PhoneNo),RTRIM(Member_ID) from RestaurantPOS_BillingInfoEB where EB_PhoneNo like '%" & txtContactNo.Text & "%' Order by BillDate"
            cmd = New SqlCommand(sql, con)
            cmd.CommandTimeout = 0
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
    ...
    Private Sub btnShowAllBills_Click(sender As System.Object, e As System.EventArgs) Handles btnShowAllBills.Click
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim sql As String = "Select RTRIM(RestaurantPOS_BillingInfoEB.Id), RTRIM(BillNo),BillDate,EBDiscountPer,EBDiscountAmt,(RestaurantPOS_BillingInfoEB.GrandTotal),Cash,Change,RTRIM(PaymentMode),RTRIM(Operator),RTRIM(CurrencyCode),ExchangeRate,RTRIM(EB_Status),RTRIM(EB_PhoneNo),RTRIM(Member_ID) from RestaurantPOS_BillingInfoEB Order by BillDate"
            cmd = New SqlCommand(sql, con)
            cmd.CommandTimeout = 0
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
