...
Public Class frmProduct
    ...
    Sub fillCategory()
        Try
            ...
            con.Open()
            adp = New SqlDataAdapter()
            adp.SelectCommand = New SqlCommand("SELECT distinct RTRIM(CategoryName) FROM RMCategory order by 1", con)
            ds = New DataSet("ds")
            adp.Fill(ds)
            dtable = ds.Tables(0)
    ...
    Private Sub DeleteRecord()
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim clx As String = "select PID from PurchaseOrder_Join,Product where Product.PID=PurchaseOrder_Join.ProductID and PID=@d1"
            cmd = New SqlCommand(clx)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", Val(txtID.Text))
            rdr = cmd.ExecuteReader()
            If rdr.Read Then
    ...
    Private Sub btnUpdate_Click(sender As System.Object, e As System.EventArgs) Handles btnUpdate.Click
        ...
        End If
        Try
            If txtProductName.Text <> txtPName.Text Then
                con = New SqlConnection(cs)
                con.Open()
                Dim ct As String = "select ProductName from Product where ProductName=@d1"
                cmd = New SqlCommand(ct)
