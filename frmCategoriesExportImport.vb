...
Public Class frmCategoriesExportImport
    Public Sub Getdata()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(CategoryName), RTRIM(Kitchen),VAT from Category order by CategoryName", con)
        ...
        If dgw.RowCount > 0 Then
            ...
            If Not Directory.Exists(fbdir) Then
                ...
                Try
                    ...
                    File.WriteAllText(fbdir & "Category.csv", csv)
                    ...
                    If (MessageBox.Show("Do you want to preview file", "Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes) Then
                        ...
                        pro.StartInfo.FileName = fbdir & "Category.csv"
                        pro.Start()
                    End If
    ...
    Private Sub Button1_Click(sender As System.Object, e As System.EventArgs) Handles btnImportExcel.Click
        Try
            ...
            If OpenFileDialog.ShowDialog() = Windows.Forms.DialogResult.OK AndAlso OpenFileDialog.FileName <> "" Then
                ...
                Dim dt As DataTable
                dtt = New DataTable()
                dtt.Columns.Add("Category Name", GetType(String))
                dtt.Columns.Add("Kitchen", GetType(String))
                dtt.Columns.Add("VAT", GetType(Double))
                ''
    ...
    Private Sub btnUpdate_Click(sender As System.Object, e As System.EventArgs) Handles btnUpdate.Click
        Try
            ...
            For Each row As DataGridViewRow In dgvGrid.Rows
                ...
                con = New SqlConnection(cs)
                con.Open()
                Dim cb As String = "update Category set Kitchen=@d2,VAT=" & Val(row.Cells(2).Value) & " where CategoryName=@d1"
            ...
            For Each row As DataGridViewRow In dgvGrid.Rows
                If Not row.IsNewRow Then
                    ...
                    If Not rdr.Read() Then
                        ...
                        con = New SqlConnection(cs)
                        con.Open()
                        Dim cb As String = "Insert Into Category (CategoryName,Kitchen,VAT,ST,SC,BackColor) Values(@d1,@d2," & Val(row.Cells(2).Value) & ",0,0,-9868951)"
                        cmd = New SqlCommand(cb)
                        cmd.Connection = con
                        cmd.Parameters.AddWithValue("@d1", row.Cells(0).Value.ToString())
                        cmd.Parameters.AddWithValue("@d2", row.Cells(1).Value.ToString())
