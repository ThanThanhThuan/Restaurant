    ...
    Private Sub dgw_MouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dgw.MouseClick
        Try
            ...
            If dgw.Rows.Count > 0 Then
                ...
                frmRecipe.txtTotalCost.Text = dr.Cells(4).Value.ToString()
                ...
                con = New SqlConnection(cs)
                con.Open()
                Dim sql As String = "Select (PID),RTRIM(ProductName),RTRIM(Convert(nvarchar(50),Quantity) + ' ' + Convert(Nvarchar(50),Unit)),RTRIM(Recipe_Join.CostPerUnit),TotalItemCost from Product,Recipe,Recipe_Join where Product.PID=Recipe_Join.ProductID and Recipe.R_ID=Recipe_Join.RecipeID and R_ID=" & dr.Cells(0).Value & " order by ProductName"
                cmd = New SqlCommand(sql, con)
                rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
                frmRecipe.dgw.Rows.Clear()
                While (rdr.Read() = True)
                    frmRecipe.dgw.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3), rdr(4))
                End While
                con.Close()
            End If
        Catch ex As Exception
