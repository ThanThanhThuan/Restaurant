...
Public Class frmKitchen_Section
    ...
    Private Sub DeleteRecord()
        ...
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim cl1 As String = "select Kitchen.KitchenName from Kitchen,StockTransfer where Kitchen.KitchenName=StockTransfer.Kitchen and Kitchen.KitchenName=@d1"
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cl As String = "select Kitchen.KitchenName from Kitchen,Category where Kitchen.KitchenName=Category.Kitchen and Kitchen.KitchenName=@d1"
            cmd = New SqlCommand(cl)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", txtKitchenName.Text)
            rdr = cmd.ExecuteReader()
            If rdr.Read Then
                MessageBox.Show("Unable to delete..Already in use in Category", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                If Not rdr Is Nothing Then
                    rdr.Close()
                End If
                Exit Sub
    ...
    Public Sub Getdata()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(KitchenName), RTRIM(Printer),RTRIM(IsEnabled) from Kitchen order by KitchenName", con)
